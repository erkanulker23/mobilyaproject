...
<script>window.__SERVER_DATA__ = {!! json_encode($data, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) !!};window.__CSRF__='{{ csrf_token() }}';window.__ADMIN_URL__='{{ url('/admin') }}';</script>
...
<script src="/awa-data.js"></script>
<script src="/awa-bridge.js"></script>
</helmet>
...
<div style="min-height:100vh;display:flex;flex-direction:column">
  ...
  <header style="{{ headerStyle }}">
    <div style="max-width:1560px;margin:0 auto;padding:0 clamp(20px,4vw,60px);height:82px;display:flex;align-items:center;justify-content:space-between;gap:28px">
      ...
      <nav style="display:flex;align-items:center;gap:clamp(18px,2.4vw,42px)">
        <sc-for list="{{ headerMenu }}" as="mi" hint-placeholder-count="6">
        <sc-if value="{{ mi.isPlain }}" hint-placeholder-val="{{ true }}">
          <span onClick="{{ mi.onClick }}" style="{{ mi.style }}">{{ mi.label }}</span>
        </sc-if>
        <sc-if value="{{ mi.isCorp }}" hint-placeholder-val="{{ false }}">
        <span style="position:relative;padding-bottom:31px;margin-bottom:-31px" onMouseEnter="{{ openCorp }}" onMouseLeave="{{ closeDrop }}">
          <span onClick="{{ mi.onClick }}" style="{{ mi.style }}">{{ mi.label }}</span>
          <sc-if value="{{ corpOpen }}" hint-placeholder-val="{{ false }}">
            <div onMouseEnter="{{ openCorp }}" onMouseLeave="{{ closeDrop }}" style="position:absolute;top:100%;left:50%;transform:translateX(-50%);margin-top:0;padding-top:10px;background:#fff;background-clip:padding-box;border:1px solid #ece6da;border-radius:14px;padding:10px;min-width:264px;box-shadow:0 24px 60px -28px rgba(23,20,15,.4);display:flex;flex-direction:column">
        ...
        <sc-if value="{{ mi.isMega }}" hint-placeholder-val="{{ false }}">
        <span style="position:relative;padding-bottom:31px;margin-bottom:-31px" onMouseEnter="{{ openCol }}" onMouseLeave="{{ closeDrop }}">
          <span onClick="{{ mi.onClick }}" style="{{ mi.style }}">{{ mi.label }}</span>
          <sc-if value="{{ colOpen }}" hint-placeholder-val="{{ false }}">
            <div onMouseEnter="{{ openCol }}" onMouseLeave="{{ closeDrop }}" style="position:fixed;left:0;right:0;top:82px;background:#fff;border-bottom:1px solid #ece6da;box-shadow:0 30px 60px -34px rgba(23,20,15,.35);padding:clamp(28px,4vw,48px) 0">
  ...
  <div>
    ...
    <sc-if value="{{ showCollections }}" hint-placeholder-val="{{ true }}">
    ...
    <sc-if value="{{ showNews }}" hint-placeholder-val="{{ true }}">
    ...
    <sc-if value="{{ showTestimonials }}" hint-placeholder-val="{{ true }}">
    <section style="background:#f6f3ed;border-top:1px solid #ece6da">
      <div style="max-width:1560px;margin:0 auto;padding:clamp(70px,9vw,120px) clamp(20px,4vw,72px)">
        <div style="margin-bottom:46px">
          <span style="{{ kickerStyle }}">{{ testiKicker }}</span>
          <h2 style="margin:16px 0 0;font-family:Archivo;font-weight:800;font-size:clamp(36px,4.4vw,58px);letter-spacing:-.025em">{{ testiTitle }}</h2>
        </div>
        <div style="display:grid;grid-template-columns:{{ g3 }};gap:26px">
          <sc-for list="{{ testimonials }}" as="tm" hint-placeholder-count="3">
            <div style="background:#fff;border:1px solid #ece6da;border-radius:18px;padding:34px;display:flex;flex-direction:column;gap:18px">
              <div style="color:#c9b98f;font-size:18px;letter-spacing:3px">{{ tm.stars }}</div>
              <p style="margin:0;font-size:16px;line-height:1.7;color:#42392c;flex:1">{{ tm.text }}</p>
              <div style="display:flex;align-items:center;gap:14px;border-top:1px solid #f0eadf;padding-top:18px">
                <div style="flex:none;width:46px;height:46px;border-radius:50%;background:#17140f;color:#fff;display:flex;align-items:center;justify-content:center;font-family:Archivo;font-weight:700;font-size:18px">{{ tm.initial }}</div>
                <div>
                  <div style="font-family:Archivo;font-weight:700;font-size:16px;color:#17140f">{{ tm.name }}</div>
                  <div style="font-family:'Space Mono';font-size:12px;color:#9a8f7e">{{ tm.company }}</div>
                </div>
              </div>
            </div>
          </sc-for>
  ...
  <footer style="background:#15120d;color:#cabfae">
    <div style="max-width:1560px;margin:0 auto;padding:clamp(56px,6vw,90px) clamp(20px,4vw,72px) clamp(40px,4vw,56px);display:grid;grid-template-columns:{{ gFooter }};gap:clamp(28px,4vw,56px)">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:12px"><sc-for list="{{ footerMenu }}" as="f" hint-placeholder-count="4"><span onClick="{{ f.onClick }}" style="cursor:pointer;font-size:15px;color:#cabfae" style-hover="color:#fff">{{ f.label }}</span></sc-for></div>
      </div>
      <div>
...
class Component extends DCLogic {
  ...
  closeDrop=()=>{ clearTimeout(this._dropT); this._dropT=setTimeout(()=>this.setState({dropdown:null}),220); };
