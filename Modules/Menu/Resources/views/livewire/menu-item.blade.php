@php
    $isMegamenu = $item->parameters->has('mega_menu') && $item->parameters->get('mega_menu') == true;
    $columns = $item->parameters->has('mega_menu_columns') ? $item->parameters->get('mega_menu_columns') : 1;
@endphp
...
<div class="item" data-id="{{ $item->id }}" wire:key="{{'menu-item-'.$item->id}}">
    ...
    >
        @foreach($item->childrenDeep as $children)
            @include('menu::livewire.menu-item', ['item' => $children])
        @endforeach
    </div>
</div>
