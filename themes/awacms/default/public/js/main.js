/* -------------------------------------------------------
 ...
 (function ($) {
    ...
    const slideAnimation = (target, options) => {
        ...
        if (!target) {
            // slideAnimation: target element is null or undefined
            return;
        }
    ...
    const slideTextAnimation = (target, options) => {
        // Check if target exists
        if (!target) {
            // slideTextAnimation: target element is null or undefined
            return;
        }
        ...
        // Check if anime-text element exists
        if (!current_anime_text) {
            // slideTextAnimation: .anime-text element not found
            return;
        }
    ...
    instagramWrapperItems.forEach(function (instagramWrapperItem) {
        var token = 'IGQWROV1oxSjhrdjlpV25LVjNfQVdpLWIwWEFzSWo4ZAHNWMEM5dGNwaEZA5a1ZAFYVJrREM0RnBkV2U5bzlSeWdxc2RtaDR5TUZAHQ21ZAOUlrVWtWMl9Xam5Ua2RkUG5HM19oNEd2b0t0VVVqdTNZAUEM3dEN4ejNJOEEZD',
...
function setupSwiper() {
    ...
    swipers.forEach(function (swiperItem) {
        ...
        if (typeof sliderOptions !== 'undefined' && sliderOptions !== null) {
            ...
            // Slide değişiminde hata kontrolleri
            sliderOptions['on'] = {
                init: function () {
                    if (!this.slides || this.slides.length === 0) {
                        // Uyarı mesajı kaldırıldı: Swiper slides yüklenmedi.
                        return;
                    }
                },
                slideChange: function () {
                    if (!this.slides || this.slides.length === 0) {
                        // Uyarı mesajı kaldırıldı: Swiper slides boş.
                        return;
                    }

                    let activeIndex = this.realIndex !== undefined ? this.realIndex : this.activeIndex;
                    if (activeIndex < 0 || activeIndex >= this.slides.length) {
                        // Geçersiz activeIndex
                        return;
                    }

                    let current_slide = this.slides[activeIndex];
                    if (!current_slide) {
                        // current_slide tanımsız
                        return;
                    }

                    let anime_el = current_slide.querySelectorAll('[data-anime]');
                    let fancy_el = current_slide.querySelectorAll('[data-fancy-text]');
            ...
            if (typeof Swiper === 'function') {
                _this.imagesLoaded(function () {
                    try {
                        ...
                        // Konsolda 'Swiper başlatıldı' mesajı kaldırıldı
                        // console.log('Swiper başlatıldı:', swiperObj);
                    } catch (error) {
                        // Swiper başlatılamadı
                    }
                });
            } else {
                // Swiper tanımlı değil
            }
        }
    });
}

// Sayfa tamamen yüklendiğinde Swiper'ı başlat
        ...
        if (marker !== null && marker !== undefined) {
            ...
            if (marker.type.toLowerCase() === 'html') {
                ...
                HTMLMarker.prototype.onAdd = function () {
                    ...
                    // Check if panes and overlayImage exist
                    if (!panes || !panes.overlayImage) {
                        // Google Maps: overlayImage pane not found
                        return;
                    }
