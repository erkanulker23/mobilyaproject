...
<section id="projeler" class="kal-section" style="position:relative;background:{{ $bgColor ?: '#F6F3ED' }};padding:130px 0">
    <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
        <div data-reveal style="opacity:0;display:flex;align-items:flex-end;justify-content:space-between;gap:30px;flex-wrap:wrap;margin-bottom:40px">
            <div>
                @if($eyebrow)<div style="display:flex;align-items:center;gap:13px;margin-bottom:18px"><span style="width:34px;height:1px;background:#9C8463"></span><span style="font-size:12px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#9C8463">{{ $eyebrow }}</span></div>@endif
                <h2 style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:clamp(32px,3.5vw,56px);line-height:1.04;letter-spacing:-.02em;color:#1C1813;max-width:16ch">{{ $title }}</h2>
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap">
                ...
                <button data-filterbtn="devam" style="font-family:'Manrope',sans-serif;font-size:13px;font-weight:700;color:#1C1813;background:transparent;border:1px solid #C9BFAD;padding:11px 22px;border-radius:30px;cursor:pointer;transition:all .3s">Devam Eden</button>
                <button data-filterbtn="tamam" style="font-family:'Manrope',sans-serif;font-size:13px;font-weight:700;color:#1C1813;background:transparent;border:1px solid #C9BFAD;padding:11px 22px;border-radius:30px;cursor:pointer;transition:all .3s">Tamamlanan</button>
            </div>
            @endif
        </div>
        ...
        <div data-grid class="kal-grid-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:22px">
            ...
            @empty
                <p style="color:#6A6358;grid-column:1/-1">Henüz ürün eklenmemiş.</p>
            @endforelse
        </div>

        <div data-reveal style="opacity:0;display:flex;justify-content:center;margin-top:48px">
            <a href="{{ $buttonUrl ?: route('projects.index') }}" style="display:inline-flex;align-items:center;gap:11px;background:#9C8463;color:#fff;font-weight:700;font-size:14px;padding:16px 32px;text-decoration:none;transition:all .35s cubic-bezier(.16,1,.3,1)" style-hover="background:#7D6A4F;transform:translateY(-3px);box-shadow:0 16px 40px rgba(156,132,99,.4)">{{ $buttonText ?: 'Tüm Ürünler' }} <span style="font-size:16px">→</span></a>
        </div>
    </div>
</section>
