...
@php
    ...
    $titleColor = $isLight ? '#1C1813' : '#fff';
    $numColor = $isLight ? '#1C1813' : '#fff';
    $labelColor = $isLight ? '#8B8273' : 'rgba(255,255,255,.62)';
    $cardBg = $isLight ? '#fff' : 'rgba(255,255,255,.045)';
    $cardBorder = $isLight ? '#E6E0D4' : 'rgba(156,132,99,.22)';
    $gridLine = $isLight ? 'rgba(43,41,38,.05)' : 'rgba(156,132,99,.06)';
...
<section class="kal-section" style="position:relative;background:{{ $kalBg }};padding:120px 0;overflow:hidden;font-family:'Manrope',system-ui,sans-serif">
  ...
  @unless($isLight)
    <div style="position:absolute;top:-120px;left:50%;transform:translateX(-50%);width:680px;height:420px;background:radial-gradient(circle,rgba(156,132,99,.16),transparent 70%);pointer-events:none"></div>
  @endunless
  ...
  <div class="kal-pad" style="position:relative;max-width:1340px;margin:0 auto;padding:0 52px">
    @if($title || $subtitle)
      <div data-reveal style="opacity:0;text-align:center;margin-bottom:64px">
        @if($subtitle)
          <div style="display:inline-flex;align-items:center;gap:13px;margin-bottom:18px"><span style="width:34px;height:1px;background:#9C8463"></span><span style="font-size:12px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#C9B79C">{{ $subtitle }}</span><span style="width:34px;height:1px;background:#9C8463"></span></div>
        @endif
        @if($title)
    ...
    <div class="kal-grid-4" style="display:grid;grid-template-columns:repeat({{ min(count($counters), 4) }},1fr);gap:20px">
      @foreach($counters as $i => $counter)
        <div data-reveal data-rd="{{ ($i % 4) * 0.1 }}" style="opacity:0;position:relative;background:{{ $cardBg }};border:1px solid {{ $cardBorder }};border-radius:18px;padding:38px 30px;overflow:hidden;transition:transform .45s cubic-bezier(.16,1,.3,1),border-color .45s,box-shadow .45s" style-hover="transform:translateY(-10px);border-color:#9C8463;box-shadow:0 26px 60px rgba(0,0,0,.22)">
          {{-- üst köşe coral aksan --}}
          <div style="position:absolute;top:0;left:0;width:100%;height:3px;background:linear-gradient(90deg,#9C8463,transparent)"></div>
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:26px">
            <div style="width:54px;height:54px;display:flex;align-items:center;justify-content:center;background:{{ $isLight ? '#1C1813' : 'rgba(156,132,99,.14)' }};color:#9C8463;font-size:21px;border-radius:13px"><i class="{{ $kalIcon($counter->icon, $i) }}"></i></div>
          ...
          <div style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:800;font-size:clamp(40px,4.4vw,58px);line-height:1;letter-spacing:-.02em;color:{{ $numColor }}"><span data-count="{{ $counter->value }}">0</span><span style="color:#9C8463">+</span></div>
          <div style="margin-top:14px;height:1px;width:42px;background:#9C8463"></div>
