...
@endif
<section class="kal-page-hero" style="position:relative;background:#1C1813;padding:130px 0 70px;overflow:hidden">
    ...
    <div class="kal-pad" style="position:relative;max-width:1340px;margin:0 auto;padding:0 52px">
        @if(!empty($breadcrumbs))
            <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-bottom:18px;font-size:12.5px;color:rgba(255,255,255,.6)">
                <a href="{{ route('home') }}" style="color:rgba(255,255,255,.6);text-decoration:none" style-hover="color:#C9B79C">Ana Sayfa</a>
                @foreach($breadcrumbs as $label => $url)
                    <span style="opacity:.5">/</span>
                    @if($url)
                        <a href="{{ $url }}" style="color:rgba(255,255,255,.6);text-decoration:none" style-hover="color:#C9B79C">{{ $label }}</a>
                    @else
        ...
        @endif
        @if(!empty($eyebrow))
            <div style="display:flex;align-items:center;gap:13px;margin-bottom:18px"><span style="width:34px;height:1px;background:#9C8463"></span><span style="font-size:12px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#DACDB8">{{ $eyebrow }}</span></div>
        @endif
