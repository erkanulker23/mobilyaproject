...
<div style="font-family:'Manrope',system-ui,sans-serif;color:#1F1C18;background:#fff">
    ...
    {{-- ARAMA --}}
    <section class="kal-section" style="background:#F6F3ED;padding:34px 0;border-bottom:1px solid #E6E0D4">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <form method="GET" action="{{ url()->current() }}" style="display:flex;gap:12px;align-items:center;max-width:560px;margin:0 auto;flex-wrap:wrap">
                <div style="flex:1;min-width:240px;display:flex;align-items:center;gap:12px;background:#fff;border:1px solid #E6E0D4;border-radius:30px;padding:6px 8px 6px 22px">
                    ...
                    <button type="submit" aria-label="Ara" style="display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border:none;border-radius:50%;background:#9C8463;color:#fff;cursor:pointer;transition:all .3s" style-hover="background:#7D6A4F">
                ...
                @if(request()->input('query'))
                    <a href="{{ url()->current() }}" style="font-size:13.5px;font-weight:700;color:#9C8463;text-decoration:none;white-space:nowrap">Temizle</a>
                @endif
            </form>
        </div>
    </section>
    ...
    <section class="kal-section" style="background:#fff;padding:90px 0 110px">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            @if(request()->input('query'))
                <p style="margin-bottom:34px;font-size:15px;color:#6A6358"><strong style="color:#1C1813">"{{ request()->input('query') }}"</strong> için sonuçlar</p>
            @endif
            ...
            <div class="kal-grid-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:22px">
                @forelse($services as $i => $service)
                    <article data-reveal data-rd="{{ ($i % 3) * 0.08 }}" itemscope itemtype="https://schema.org/Service" style="opacity:0;display:flex;flex-direction:column;background:#fff;border:1px solid #E6E0D4;border-radius:14px;overflow:hidden;transition:transform .4s cubic-bezier(.16,1,.3,1),box-shadow .4s,border-color .4s" style-hover="transform:translateY(-8px);box-shadow:0 24px 50px rgba(43,41,38,.1);border-color:#9C8463">
                        <a href="{{ route('services.show', $service->slug) }}" aria-label="{{ $service->title }}" style="position:relative;display:block;aspect-ratio:16/10;overflow:hidden;background:#0c1018">
                            ...
                            @else
                                <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#3B342D,#1C1813);color:#C9B79C;font-family:'Plus Jakarta Sans';font-weight:800;font-size:42px">{{ mb_substr($service->title, 0, 1) }}</div>
                            @endif
                        ...
                        <div style="display:flex;flex-direction:column;flex:1;padding:28px 26px">
                            <h2 itemprop="name" style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:21px;line-height:1.2;color:#1C1813">
                                <a href="{{ route('services.show', $service->slug) }}" style="color:#1C1813;text-decoration:none">{{ $service->title }}</a>
                            </h2>
                            ...
                            @endif
                            <a href="{{ route('services.show', $service->slug) }}" style="margin-top:22px;display:inline-flex;align-items:center;gap:9px;font-size:13.5px;font-weight:700;color:#9C8463;text-decoration:none" style-hover="color:#7D6A4F">
                                Detaylı Bilgi
                ...
                @empty
                    <div style="grid-column:1/-1;text-align:center;padding:70px 0">
                        <h3 style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:26px;color:#1C1813">Yakında</h3>
    ...
    {{-- SÜREÇ --}}
    <section class="kal-section" style="background:#F6F3ED;padding:110px 0">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div data-reveal style="opacity:0;text-align:center;margin-bottom:60px">
                <div style="display:inline-flex;align-items:center;gap:13px;margin-bottom:20px"><span style="width:34px;height:1px;background:#9C8463"></span><span style="font-size:12px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#9C8463">Nasıl Çalışıyoruz</span><span style="width:34px;height:1px;background:#9C8463"></span></div>
                <h2 style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:clamp(30px,3.2vw,50px);line-height:1.06;letter-spacing:-.02em;color:#1C1813">Dört adımda projeniz</h2>
            </div>
            <div class="kal-grid-4" style="display:grid;grid-template-columns:repeat(4,1fr);gap:22px">
                ...
                @foreach($steps as $i => $step)
                    <div data-reveal data-rd="{{ $i * 0.06 }}" style="opacity:0">
                        ...
                        <h3 style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:19px;color:#1C1813;margin-top:8px">{{ $step[1] }}</h3>
    ...
    <section class="kal-section" style="background:#fff;padding:120px 0">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div style="background:linear-gradient(135deg,#1C1813,#3B342D);padding:72px 56px;border-radius:18px;text-align:center;position:relative;overflow:hidden">
                <div style="position:absolute;top:-30%;right:-6%;width:380px;height:380px;background:radial-gradient(circle,rgba(156,132,99,.35),transparent 65%);pointer-events:none"></div>
                ...
                <div style="position:relative;display:flex;gap:16px;justify-content:center;flex-wrap:wrap;margin-top:32px">
                    <a href="{{ route('contact.index') }}" style="display:inline-flex;align-items:center;gap:12px;background:#9C8463;color:#fff;font-weight:700;font-size:14px;letter-spacing:.4px;padding:18px 34px;text-decoration:none;transition:all .35s cubic-bezier(.16,1,.3,1)" style-hover="background:#fff;color:#1C1813;transform:translateY(-3px)">İletişime Geçin →</a>
