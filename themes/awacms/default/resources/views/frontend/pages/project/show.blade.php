...
<div style="font-family:'Manrope',system-ui,sans-serif;color:#1F1C18;background:#fff">
    ...
    {{-- HERO --}}
    <section style="position:relative;background:#1C1813;min-height:82vh;display:flex;align-items:flex-end;overflow:hidden">
        ...
        <div class="kal-pad" style="position:relative;max-width:1340px;margin:0 auto;padding:0 52px 64px;width:100%">
            <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-bottom:20px;font-size:12.5px;color:rgba(255,255,255,.6)">
                ...
                <span style="color:#C9B79C">{{ $project->title }}</span>
            </div>
    ...
    {{-- KÜNYE ŞERİDİ --}}
    <section style="background:#17140F;color:#fff">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:1px;background:rgba(255,255,255,.08)">
                @foreach($baseInfo as $label => $value)
                    <div style="background:#17140F;padding:26px 24px">
                        <div style="font-size:11px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:#C9B79C">{{ $label }}</div>
    ...
    <section class="kal-section" style="background:#fff;padding:110px 0">
        <div class="kal-pad kal-split" style="max-width:1340px;margin:0 auto;padding:0 52px;display:grid;grid-template-columns:1.4fr .8fr;gap:64px;align-items:start">
            <div>
                <div style="display:flex;align-items:center;gap:13px;margin-bottom:22px"><span style="width:34px;height:1px;background:#9C8463"></span><span style="font-size:12px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#9C8463">Proje Hakkında</span></div>
                @if($project->short_description)
                    <p style="font-size:19px;line-height:1.7;color:#1C1813;font-weight:500;margin-bottom:22px">{{ $project->short_description }}</p>
                @endif
            ...
            @if($kunye->count())
            <aside style="background:#F6F3ED;border:1px solid #E6E0D4;padding:32px 30px;border-radius:14px;position:sticky;top:100px">
                <div style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:18px;color:#1C1813;margin-bottom:20px">Teknik Künye</div>
                <div style="display:flex;flex-direction:column;gap:14px">
                    @foreach($kunye as $spec)
                        <div style="display:flex;justify-content:space-between;gap:16px;padding-bottom:14px;border-bottom:1px solid #E6E0D4">
                            ...
                            <span style="font-size:13.5px;font-weight:700;color:#1C1813;text-align:right">{{ $spec['value'] ?? '' }}</span>
                        </div>
                    @endforeach
                </div>
                <a href="{{ route('contact.index') }}" style="margin-top:24px;display:flex;align-items:center;justify-content:center;gap:10px;background:#9C8463;color:#fff;font-weight:700;font-size:14px;padding:15px;text-decoration:none;transition:all .3s" style-hover="background:#7D6A4F">Bilgi Alın →</a>
            </aside>
            @endif
        </div>
    </section>
        ...
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div class="kal-grid-4" style="display:grid;grid-template-columns:repeat(4,1fr);gap:18px">
                @foreach([
                    ...
                    <div data-reveal data-rd="{{ $i * 0.06 }}" style="opacity:0;background:#fff;border:1px solid #E6E0D4;border-radius:14px;padding:30px 26px;transition:transform .4s cubic-bezier(.16,1,.3,1),box-shadow .4s,border-color .4s" style-hover="transform:translateY(-8px);box-shadow:0 24px 50px rgba(43,41,38,.1);border-color:#9C8463">
                        <div style="width:50px;height:50px;display:flex;align-items:center;justify-content:center;background:#1C1813;color:#9C8463;font-size:20px;border-radius:11px;margin-bottom:22px"><i class="fa-solid {{ $adv[0] }}"></i></div>
                        <h3 style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:18px;color:#1C1813">{{ $adv[1] }}</h3>
    ...
    <section class="kal-section" style="background:#F6F3ED;padding:100px 0">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div data-reveal style="opacity:0;display:flex;align-items:center;gap:13px;margin-bottom:14px"><span style="width:34px;height:1px;background:#9C8463"></span><span style="font-size:12px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:#9C8463">Proje Galerisi</span></div>
            <h2 data-reveal style="opacity:0;font-family:'Plus Jakarta Sans';font-weight:800;font-size:clamp(28px,3vw,44px);color:#1C1813;margin-bottom:40px">Görsellere göz atın</h2>
    ...
    <section class="kal-section" style="background:#fff;padding:100px 0">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:36px;gap:20px;flex-wrap:wrap">
                <h2 style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:clamp(26px,2.8vw,42px);color:#1C1813">Diğer Projeler</h2>
                <a href="{{ route('projects.index') }}" style="font-size:13.5px;font-weight:700;color:#9C8463;text-decoration:none;border-bottom:2px solid #9C8463;padding-bottom:5px">Tümünü Gör →</a>
            </div>
    ...
    {{-- CTA --}}
    <section class="kal-section" style="position:relative;background:#1C1813;padding:90px 0;overflow:hidden">
        <div style="position:absolute;inset:0;background-image:linear-gradient(rgba(156,132,99,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(156,132,99,.05) 1px,transparent 1px);background-size:54px 54px;-webkit-mask-image:radial-gradient(80% 70% at 50% 40%,#000,transparent);mask-image:radial-gradient(80% 70% at 50% 40%,#000,transparent);pointer-events:none"></div>
        <div class="kal-pad" style="position:relative;max-width:1340px;margin:0 auto;padding:0 52px;display:flex;align-items:center;justify-content:space-between;gap:30px;flex-wrap:wrap">
            ...
            <div style="display:flex;gap:14px;flex-wrap:wrap">
                <a href="{{ route('contact.index') }}" style="display:inline-flex;align-items:center;gap:10px;background:#9C8463;color:#fff;font-weight:700;font-size:14px;padding:17px 30px;text-decoration:none;transition:all .35s" style-hover="background:#7D6A4F;transform:translateY(-3px)">İletişime Geçin <i class="fa-solid fa-arrow-right"></i></a>
