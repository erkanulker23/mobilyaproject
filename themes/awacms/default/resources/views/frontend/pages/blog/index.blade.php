...
<div style="font-family:'Manrope',system-ui,sans-serif;color:#1F1C18;background:#fff">
    ...
    {{-- ARAMA + KATEGORİLER --}}
    <section class="kal-section" style="background:#F6F3ED;padding:46px 0;border-bottom:1px solid #E6E0D4">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:24px;flex-wrap:wrap">
                ...
                <form method="GET" action="{{ route('blog.index') }}" style="flex:1;min-width:260px;max-width:440px;display:flex;align-items:center;background:#fff;border:1px solid #E6E0D4;border-radius:32px;padding:6px 6px 6px 22px">
                    ...
                    <button type="submit" aria-label="Ara" style="display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border:none;border-radius:50%;background:#9C8463;color:#fff;cursor:pointer;transition:background .3s" style-hover="background:#7D6A4F">
    ...
    <section class="kal-section" style="background:#fff;padding:80px 0 120px">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div class="kal-grid-3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:22px">
                @forelse($posts as $i => $post)
                    <a data-reveal data-rd="{{ ($i % 3) * 0.08 }}" href="{{ route('blog.post.show', $post->slug) }}" style="opacity:0;text-decoration:none;background:#fff;border:1px solid #E6E0D4;border-radius:14px;overflow:hidden;transition:transform .4s cubic-bezier(.16,1,.3,1),box-shadow .4s" style-hover="transform:translateY(-8px);box-shadow:0 24px 50px rgba(43,41,38,.1)">
                        ...
                        <div style="padding:24px 24px 28px">
                            <div style="font-size:11.5px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#9C8463">{{ optional($post->publishAt)->translatedFormat('d F Y') }}</div>
                            <h2 style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:19px;color:#1C1813;margin-top:10px;line-height:1.25">{{ $post->title }}</h2>
                            ...
                            <span style="display:inline-flex;align-items:center;gap:8px;margin-top:18px;font-size:13px;font-weight:700;color:#9C8463">Devamını Oku
                ...
                @empty
                    <div style="grid-column:1/-1;text-align:center;padding:80px 20px;background:#F6F3ED;border:1px solid #E6E0D4;border-radius:16px">
                        <div style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:26px;color:#1C1813">Yakında</div>
