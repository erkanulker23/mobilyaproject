...
<div style="font-family:'Manrope',system-ui,sans-serif;color:#1F1C18;background:#fff">
    ...
    {{-- HERO --}}
    <section style="position:relative;background:#1C1813;min-height:56vh;display:flex;align-items:flex-end;overflow:hidden">
        ...
        <div class="kal-pad" style="position:relative;max-width:1340px;margin:0 auto;padding:0 52px 60px;width:100%">
            <div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-bottom:20px;font-size:12.5px;color:rgba(255,255,255,.6)">
                ...
                <a href="{{ route('blog.index') }}" style="color:rgba(255,255,255,.6);text-decoration:none" style-hover="color:#C9B79C">Haberler</a><span style="opacity:.5">/</span>
                <span style="color:#C9B79C">{{ $post->title }}</span>
            </div>
            @if($post->category)
            <div style="display:inline-flex;margin-bottom:16px;font-size:10.5px;font-weight:700;letter-spacing:.7px;text-transform:uppercase;color:#fff;background:#9C8463;padding:7px 14px;border-radius:20px">{{ $post->category->name }}</div>
            @endif
    ...
    <section class="kal-section" style="background:#fff;padding:90px 0">
        <div class="kal-pad" style="max-width:880px;margin:0 auto;padding:0 52px">
            @if($post->shortDescription)
                <p style="font-size:19px;line-height:1.7;color:#1C1813;font-weight:500;margin-bottom:30px;padding-bottom:30px;border-bottom:1px solid #E6E0D4">{{ strip_tags($post->shortDescription) }}</p>
            @endif
            ...
            <div style="margin-top:48px;padding-top:30px;border-top:1px solid #E6E0D4;display:flex;align-items:center;gap:14px;flex-wrap:wrap">
                ...
                <a href="https://linkedin.com/sharing/share-offsite/?url={{ urlencode(request()->fullUrl()) }}" target="_blank" rel="noopener" aria-label="LinkedIn'da paylaş" style="display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:50%;background:#F6F3ED;border:1px solid #E6E0D4;color:#1C1813;text-decoration:none;transition:all .3s" style-hover="background:#9C8463;color:#fff;border-color:#9C8463"><i class="fab fa-linkedin-in"></i></a>
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}" target="_blank" rel="noopener" aria-label="Facebook'ta paylaş" style="display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:50%;background:#F6F3ED;border:1px solid #E6E0D4;color:#1C1813;text-decoration:none;transition:all .3s" style-hover="background:#9C8463;color:#fff;border-color:#9C8463"><i class="fab fa-facebook-f"></i></a>
                <a href="https://x.com/intent/post?text={{ urlencode($post->title) }}&url={{ urlencode(request()->fullUrl()) }}" target="_blank" rel="noopener" aria-label="X'te paylaş" style="display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:50%;background:#F6F3ED;border:1px solid #E6E0D4;color:#1C1813;text-decoration:none;transition:all .3s" style-hover="background:#9C8463;color:#fff;border-color:#9C8463"><i class="fab fa-x-twitter"></i></a>
                <a href="mailto:?subject={{ urlencode($post->title) }}&body={{ urlencode('Bu haberi okumanızı tavsiye ederim: '.request()->fullUrl()) }}" aria-label="E-posta ile paylaş" style="display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:50%;background:#F6F3ED;border:1px solid #E6E0D4;color:#1C1813;text-decoration:none;transition:all .3s" style-hover="background:#9C8463;color:#fff;border-color:#9C8463"><i class="fas fa-envelope"></i></a>
            </div>
        </div>
    </section>

    {{-- YORUMLAR --}}
    <section class="kal-section" style="background:#F6F3ED;padding:90px 0;border-top:1px solid #E6E0D4">
        <div class="kal-pad" style="max-width:880px;margin:0 auto;padding:0 52px">
            @php $comments = $post->comments; @endphp
            <h2 style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:clamp(24px,2.6vw,36px);color:#1C1813;margin-bottom:8px">Yorumlar</h2>
            ...
            <div style="background:#fff;border:1px solid #E6E0D4;border-radius:16px;padding:32px 30px;margin-bottom:40px">
                <div style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:18px;color:#1C1813;margin-bottom:20px">Yorum Yap</div>
                <form id="kal-comment-form" method="POST" action="{{ route('blog.comments.store', $post->slug) }}" data-post-id="{{ $post->id }}">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px" class="kal-split">
                        <input type="text" name="fullname" placeholder="Adınız Soyadınız" required style="width:100%;padding:14px 16px;border:1px solid #E6E0D4;border-radius:10px;background:#F6F3ED;font-family:'Manrope',sans-serif;font-size:14.5px;color:#1F1C18;outline:none">
                        <input type="email" name="email" placeholder="E-posta Adresiniz" required style="width:100%;padding:14px 16px;border:1px solid #E6E0D4;border-radius:10px;background:#F6F3ED;font-family:'Manrope',sans-serif;font-size:14.5px;color:#1F1C18;outline:none">
                    </div>
                    <textarea name="comment" rows="4" placeholder="Yorumunuzu buraya yazın..." required style="width:100%;padding:14px 16px;border:1px solid #E6E0D4;border-radius:10px;background:#F6F3ED;font-family:'Manrope',sans-serif;font-size:14.5px;color:#1F1C18;outline:none;resize:vertical;margin-bottom:18px"></textarea>
                    <button type="submit" style="display:inline-flex;align-items:center;gap:10px;background:#9C8463;color:#fff;font-family:'Manrope',sans-serif;font-weight:700;font-size:14px;padding:15px 30px;border:none;border-radius:10px;cursor:pointer;transition:background .3s" style-hover="background:#7D6A4F">Yorum Gönder →</button>
                    ...
                    @foreach($comments as $comment)
                        ...
                        <div style="background:#fff;border:1px solid #E6E0D4;border-radius:14px;padding:24px 26px">
                            <div style="display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-bottom:10px">
                                <div style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:15.5px;color:#1C1813">{{ $comment->fullname }}</div>
                            ...
                            <div style="margin-top:18px;padding-left:22px;border-left:3px solid #DACDB8;display:flex;flex-direction:column;gap:14px">
                                @foreach($comment->children as $reply)
                                    @if($reply->is_approved)
                                    <div style="background:#F6F3ED;border-radius:10px;padding:16px 18px">
                                        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:7px">
                                            <div style="font-weight:700;font-size:14px;color:#1C1813">{{ $reply->fullname }}</div>
    ...
    <section class="kal-section" style="background:#fff;padding:100px 0">
        <div class="kal-pad" style="max-width:1340px;margin:0 auto;padding:0 52px">
            <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:36px;gap:20px;flex-wrap:wrap">
                <h2 style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:clamp(26px,2.8vw,42px);color:#1C1813">İlgili Haberler</h2>
                <a href="{{ route('blog.index') }}" style="font-size:13.5px;font-weight:700;color:#9C8463;text-decoration:none;border-bottom:2px solid #9C8463;padding-bottom:5px">Tümünü Gör →</a>
            </div>
                ...
                @foreach($relatedPosts as $i => $rp)
                    ...
                    <a data-reveal data-rd="{{ ($i % 3) * 0.08 }}" href="{{ route('blog.post.show', $rpSlug) }}" style="opacity:0;text-decoration:none;background:#fff;border:1px solid #E6E0D4;border-radius:14px;overflow:hidden;transition:transform .4s cubic-bezier(.16,1,.3,1),box-shadow .4s" style-hover="transform:translateY(-8px);box-shadow:0 24px 50px rgba(43,41,38,.1)">
                        ...
                        <div style="padding:24px 24px 28px">
                            <div style="font-size:11.5px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#9C8463">{{ optional($rpDate)->translatedFormat('d F Y') }}</div>
                            <h3 style="font-family:'Plus Jakarta Sans';font-weight:700;font-size:18px;color:#1C1813;margin-top:10px;line-height:1.25">{{ $rpTitle }}</h3>
